...
sed 's/#.*//' ${GIT_DIR}/Requirements/GassistPi-system-requirements.txt | xargs apt-get install -y
pip install pyaudio
...
if [[ $osversion = "Raspbian Buster" ]]; then
  cp -f ${GIT_DIR}/src/resources/Buster-wrapper/_snowboydetect.so ${GIT_DIR}/src/_snowboydetect.so
  cp -f ${GIT_DIR}/src/resources/Buster-wrapper/snowboydetect.py ${GIT_DIR}/src/snowboydetect.py
elif [[ $osversion = "Raspbian Stretch" ]]; then
  cp -f ${GIT_DIR}/src/resources/Stretch-wrapper/_snowboydetect.so ${GIT_DIR}/src/_snowboydetect.so
  cp -f ${GIT_DIR}/src/resources/Stretch-wrapper/snowboydetect.py ${GIT_DIR}/src/snowboydetect.py
fi
...
if [[ $osversion != "Raspbian Stretch" ]] && [[ $osversion != "Raspbian Buster" ]]; then
  ...
  if [ ! -d /home/${USER}/programs/libraries/swig/ ]; then
    mkdir -p programs/libraries/ && cd programs/libraries
  ...
  if [ -e /home/${USER}/programs/snowboy/swig/Python3/_snowboydetect.so ]; then
    echo "Copying Snowboy files to GassistPi directory"
    cp -f ./_snowboydetect.so ${GIT_DIR}/src/_snowboydetect.so
    cp -f ./snowboydetect.py ${GIT_DIR}/src/snowboydetect.py
  else
